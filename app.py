...
def buildGantt(core_projects, today):
  ...
  for index, row in core_projects.iterrows():
    if ('Day' in row['project'] or 'Rush' in row['project']):
      core_projects = core_projects.drop(index)
  core_projects = core_projects.reset_index()
  ...
  for obj in list(df):
    if ('Day' in obj['Project'] or 'Rush' in obj['Project']):
      df.remove(obj)
      ...
      if user_id != 0:
        ...
        if not projects.empty:
          ...
          if core_projects.shape[0] > 0:
            ...
            core_projects = core_projects.reset_index(drop = True)
          bar.progress(80)
          fig, core_projects = buildGantt(core_projects, today)
          bar.progress(90)
